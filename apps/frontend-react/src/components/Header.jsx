...
export default function Header({
  ...
  const normalizedSelectedMarket = String(selectedMarket || "INTERNATIONAL").toUpperCase();
  ...
  const filteredAccounts = (me?.accounts || []).filter(
    (account) => String(account.market_type || "INTERNATIONAL").toUpperCase() === normalizedSelectedMarket
  );
  ...
  const primaryNavItems = useMemo(() => {
    const preferred = Array.isArray(primaryNavIds) ? primaryNavIds : [];
    ...
    for (const item of navItems) {
      if (!resolved.some((entry) => entry.id === item.id)) {
        resolved.push(item);
      }
      if (resolved.length >= 2) break;
    }
    return resolved.slice(0, 2);
  }, [navItems, primaryNavIds]);
  const overflowNavItems = navItems.filter((item) => !primaryNavItems.some((primaryItem) => primaryItem.id === item.id));
  const overflowActive = overflowNavItems.some((item) => item.id === currentPage);
    ...
    <header className="flex flex-col gap-0.5 rounded-2xl border border-indigo-700 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 px-2 py-0 text-white shadow-lg lg:flex-row lg:items-center lg:justify-between lg:px-2.5 lg:py-0">
      <div className="flex min-h-[2rem] items-center justify-between gap-1 lg:min-h-0">
        ...
        <div className="flex flex-1 items-center justify-center lg:flex-none lg:justify-start">
          <div className="flex items-center gap-1">
            <img
              src="/signalbridge-logo-dark-cropped.svg"
              alt="SignalBridge"
            ...
            {normalizedSelectedMarket === "INDIAN" && hasSelectedMarketAccount ? (
            ...
            {normalizedSelectedMarket === "INTERNATIONAL" && Number.isFinite(Number(runningPl)) ? (
          ...
          <div className="hidden items-center gap-1 rounded-xl bg-white/10 p-0.5 lg:flex">
            ...
            {overflowNavItems.length > 0 ? (
              <div className="relative">
                <button
                  type="button"
                  onClick={() => setOverflowNavOpen((current) => !current)}
                  className={`rounded-lg px-3 py-0.5 text-sm font-semibold transition ${overflowNavOpen || overflowActive ? "bg-white text-slate-900" : "text-indigo-100"}`}
                  aria-label="More pages"
                >
                  &gt;&gt;
                </button>
                {overflowNavOpen ? (
                  <div className="absolute left-0 top-full z-30 mt-2 w-56 rounded-2xl border border-indigo-100 bg-white p-2 text-slate-900 shadow-2xl">
                    <div className="space-y-1">
                      {overflowNavItems.map((item) => (
                        <button
                          ...
                          className={`w-full rounded-xl px-3 py-2 text-left text-sm font-semibold transition ${currentPage === item.id ? "bg-indigo-600 text-white" : "bg-slate-50 text-slate-800 hover:bg-slate-100"}`}
                        >
                          {item.label}
                        </button>
                      ))}
      ...
      {me && (
        <div className="hidden flex-col gap-1 sm:flex-row sm:items-center lg:flex">
          ...
          <button onClick={onOpenManageAccount} className="rounded-xl bg-indigo-500 px-3 py-1 text-sm font-semibold hover:bg-indigo-600">
            Manage Account
          </button>
          <div className="relative">
            <button
